...
class Teacher():
    ...
    def solver(self, hinged, roll, roll_direction, fixed, moment, point, q, qi, node_map, target_dir):
        self.reset()
        if (len(hinged) == 1 and len(roll) == 1) or len(fixed) == 1:
            ...
            self.savior(hinged, roll, roll_direction, fixed, node_map, moment, df, target_dir)
                    ...
                    with doc.create(Subsubsection(f'Momento Fletor')):
                        ...
                        if c != 0:
                            ...
                            if f'{c+1}: ' in str(moment):
                                doc.append(NoEscape(self.symbol_filter(f"c = {const} + {self.get_signal_constant(moment.get(c+1), -eq[c][3], numeric_const):.2f}")))
                            else:
                                doc.append(NoEscape(self.symbol_filter(f"c = {const}")))
                            doc.append(NoEscape(r'\end{dmath*}'))
                        elif c == 0 and f'{c+1}: ' in str(moment):
                            doc.append(NoEscape(r'\begin{dmath*}'))
                            doc.append(self.symbol_filter(NoEscape(f'c = {self.get_signal_constant(moment.get(c+1), -eq[c][3]):.2f}')))
                            doc.append(NoEscape(r'\end{dmath*}'))
                        else:
