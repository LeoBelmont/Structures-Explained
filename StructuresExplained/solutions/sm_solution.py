...
class Teacher():
    ...
    def solver(self, hinged, roll, roll_direction, fixed, moment, point, q, qi, node_map, target_dir, language):
        self.reset()
        if (len(hinged) == 1 and len(roll) == 1) or len(fixed) == 1:
            ...
            for c in range(1, len(node_map) + 1):
                ...
                savefig(f'tmp\\figs\\structure{c}')

            plt.style.use('dark_background')
            self.generatePDF(hinged, roll, roll_direction, fixed, node_map, moment, df, language, target_dir)
    ...
    def generatePDF(self, hinged, roll, roll_direction, fixed, node_map, moment, df, language, target_dir):
        doc = Document(document_options="a4paper,12pt", documentclass="article")
        doc.preamble.append(NoEscape(header.PDFsettings))

        tpdf = translate_PDF_structure(language)
            ...
            if fixed:
                ...
                with doc.create(Subsection(tpdf.fixed_EFY)):
                    with doc.create(Alignat(escape=False, numbering=False)) as sum_Fy:
                        ...
                        sum_Fy.append(
                            f'{self.total_point_y + self.total_load_y} {self.get_signal(self.EFy)} Fy = 0' r'\\')
                ...
                with doc.create(Subsection(tpdf.fixed_EFX)):
                    with doc.create(Alignat(escape=False, numbering=False)) as sum_Fx:
                        ...
                        sum_Fx.append(
                            f'{self.total_point_x + self.total_load_x} {self.get_signal(self.EFx)} Fx = 0' r'\\')
                ...
                with doc.create(Subsection(tpdf.fixed_moment)):
                    ...
                    if roll_angle == math.pi / 2:
                        ...
                        doc.append(NoEscape(self.symbol_filter(
                            f'{self.total_moment + self.total_point} {self.get_signal(self.Bx)} Bx \cdot {self.roll_dist_y} = 0 \\\\')))
                        doc.append(NoEscape(r'\end{dmath*}'))
                        doc.append(NoEscape(r'\begin{dmath*}'))
                        ...
                        doc.append(NoEscape(r'\begin{dmath*}'))
                        doc.append(NoEscape(self.symbol_filter(
                            f'{self.total_moment + self.total_point} {self.get_signal(self.By)} By \cdot {self.roll_dist_x} = 0 \\\\')))
                        doc.append(NoEscape(r'\end{dmath*}'))
                        doc.append(NoEscape(r'\begin{dmath*}'))
                    ...
                    with doc.create(Alignat(escape=False, numbering=False)) as sum_Fy:
                        ...
                        if round(float(self.By), 2) != 0:
                            sum_Fy.append(self.symbol_filter(
                                f'{self.total_point_y}{self.total_load_y} + {float(self.By):.2f} {self.get_signal(self.Ay)} Ay = 0' r'\\'))
                        else:
                            sum_Fy.append(self.symbol_filter(
                                f'{self.total_point_y}{self.total_load_y} {self.get_signal(self.Ay)} Ay = 0' r'\\'))
                    ...
                    with doc.create(Alignat(escape=False, numbering=False)) as sum_Fx:
                        ...
                        if float(f'{float(self.Bx):.2f}') != 0:
                            sum_Fx.append(self.symbol_filter(
                                f'{self.total_point_x}{self.total_load_x} + {float(self.Bx):.2f} {self.get_signal(self.Ax)} Ax = 0' r'\\'))
                        else:
                            sum_Fx.append(self.symbol_filter(
                                f'{self.total_point_x}{self.total_load_x} {self.get_signal(self.Ax)} Ax = 0' r'\\'))
                    ...
                    with doc.create(Subsubsection(tpdf.step_bending_stress)):
                        ...
                        if c != 0:
                            ...
                            if f'{c + 1}: ' in str(moment):
                                doc.append(NoEscape(self.symbol_filter(
                                    f"c = {const} + {self.get_signal_constant(moment.get(c + 1), -eq[c][3], numeric_const):.2f}")))
                            else:
                            ...
                            doc.append(self.symbol_filter(
                                NoEscape(f'c = {self.get_signal_constant(moment.get(c + 1), -eq[c][3]):.2f}')))
                            doc.append(NoEscape(r'\end{dmath*}'))
                        else:
        ...
        # doc.generate_tex(r'tmp\resolucao')
        # doc.generate_pdf(os.path.abspath(target_dir), compiler='pdflatex', win_no_console=True)
