...
class Ui_tenshi(QWidget):
    ...
    def configUI(self, tenshi):
        ...
        self.circle_apply.installEventFilter(self)
    ...
    def retranslateUi(self):
        ...
        self.toolButton_11.setText(_translate("tenshi", "Tensão de \n"
                                                        ...
                                                        "mento\n"
                                                        "e Fluxo"))
        self.toolButton_8.setToolTip(_translate("tenshi", "Adicionar semicírculos"))
        ...
        self.qload_warning = "Os valores inicial e final precisam ter o mesmo sinal"
        ...
        self.close_title = "Sair sem Salvar?"
        self.close_text = "Tem certeza que deseja sair? É possível que haja estruturas não salvas."
    ...
    def retranslateUiEN(self):
        ...
        self.qload_warning = "Initial and final values must have the same signal"
        ...
        self.close_title = "Quit without saving?"
        self.close_text = "Are you sure you want to quit? It's possible there are unsaved structures."
        ...
        try:
            if int(self.qload_pos.text()) in self.ss.node_map.keys():
                if float(self.qload_initial.text()) >= 0 and float(self.qload_final.text()) >= 0 or \
                    ...
                    msg.setText(self.qload_warning)
            ...
            if self.mohr.sx is not None:
                ...
                if ok:
                    try:
                        ...
                        self.loadingScreen.exec_()
                        if not self.loadingUi.userTerminated:
                            os.replace("tmp\\resolucaomohr.pdf", f"{file}")
                            self.pdf_generated_prompt()
                        self.deleteTempFolder()
                    except:
                        self.latex_packages_warning()
    ...
    def structure_savefig(self):
        ...
        savefig("tmp\\figs\\structure", transparent=True)
        self.solve()
        self.ss.show_structure(show=False, diagram=2)
        savefig("tmp\\figs\\diagram2", transparent=True)
        self.ss.show_structure(show=False, diagram=3)
        savefig("tmp\\figs\\diagram1", transparent=True)
        self.ss.show_reaction_force(show=False)
        savefig("tmp\\figs\\supports", transparent=True)
        self.ss.show_axial_force(show=False)
        savefig("tmp\\figs\\axial", transparent=True)
        self.ss.show_shear_force(show=False)
        savefig("tmp\\figs\\shear", transparent=True)
        self.ss.show_bending_moment(show=False)
        savefig("tmp\\figs\\moment", transparent=True)
    ...
    def mohr_savefig(self):
        ...
        figure.savefig("tmp\\figs\\mohrfig", transparent=True)
        plt.style.use('dark_background')
        self.last_figure.click()
    ...
    def get_prompt_values(self):
        ...
        prompt.setupUi(load_prompt, self.language)
        load_prompt.exec_()
    ...
    def doStartUpStuff(self):
        ...
        self.sy.setText("20")
        self.txy.setText("30")
        self.mohr_apply.click()
        self.visualize_structure()
        self.last_figure = self.show_structure

    def eventFilter(self, obj, event):
        if event.type() == QtCore.QEvent.HoverEnter:
            self.label_128.setHidden(False)
            return True
    ...
    def setupLoading(self):
        ...
        self.loadingUi = loadingPrompt.Ui_loading_prompt(self.language, self.loadingScreen)
        self.loadingUi.setupUi()

    def makeTempFolder(self):
        if not os.path.isdir("tmp"):
            os.mkdir("tmp")
        if not os.path.isdir("tmp\\figs"):
